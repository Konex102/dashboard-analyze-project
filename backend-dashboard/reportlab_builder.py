...
class _NumberedCanvas(_RLCanvas):
    """Deferred showPage so total pages are known at save() time."""
    ...
    def showPage(self):                          # called per page by ReportLab
        self._saved_states.append(dict(self.__dict__))
        self._startPage()

    def save(self):                              # called once at very end
...
def _draw_running_layer(c, hd: dict, page_num: int, total: int) -> None:
    ...
    c.setLineWidth(0.8)
    c.rect(ML, band_y, UW, band_h, fill=0, stroke=0)
    ...
    # Left: file display name (truncated)
    c.setFont("Helvetica", 7)
    c.setFillColor(DARK)
    c.drawString(ML, footer_y + 0.2 * cm,
                 str(hd.get("display_name", ""))[:55])
...
def _fig_to_png(fig: go.Figure, w: int = 680, h: int = 290) -> bytes | None:
    try:
...
def _png_to_rl_image(
    png: bytes, width_pt: float, height_pt: float
) -> Image | None:
    if not png:
        return None
    try:
...
# Reusable section header flowable
...
_CHART_LAYOUT = dict(
    ...
    margin      = dict(l=40, r=12, t=32, b=55),
    legend      = dict(orientation="h", y=-0.38, font=dict(size=8)),
    paper_bgcolor = "white",
    plot_bgcolor  = "white",
    height        = 290,
)
    ...
    try:
        ...
        for i, col in enumerate(y_cols):
            fig.add_trace(go.Scatter(
                x=d["_x"], y=d[col], mode="lines", name=col,
                line=dict(color=_PAL[i % len(_PAL)], width=1.6),
            ))
...
def _build_bar(
    ...
    try:
        ...
        fig.update_layout(title=title, barmode="group", **_CHART_LAYOUT)
        return fig
    except Exception:
        return None
...
def _build_area(
    ...
    try:
        ...
        for i, col in enumerate(y_cols):
            fig.add_trace(go.Scatter(
                x=d["_x"], y=d[col], mode="lines",
                fill="tozeroy" if i == 0 else "tonexty",
                name=col, line=dict(color=_PAL[i % len(_PAL)], width=1.4),
            ))
        fig.update_layout(title=title, **_CHART_LAYOUT)
        return fig
    except Exception:
...
def _build_scatter(
    ...
    try:
        ...
        for i, col in enumerate(y_cols):
            fig.add_trace(go.Scatter(
                x=df[x_col], y=df[col], mode="markers", name=col,
                marker=dict(color=_PAL[i % len(_PAL)], size=4, opacity=0.7),
            ))
        fig.update_layout(title=title, **_CHART_LAYOUT)
        return fig
    except Exception:
...
def _build_spv_bar(
    df: pd.DataFrame, sp_col: str, pv_col: str, label: str
) -> go.Figure | None:
    try:
        d = df.copy()
        ...
        fig = go.Figure(go.Bar(
            x=["Dalam SP", "Lebih Rendah", "Lebih Tinggi"],
            y=[normal, lower, higher],
            marker_color=["#3B82F6", "#F59E0B", "#EF4444"],
            text=[f"{v:,}<br>({v/total*100:.1f}%)" for v in [normal, lower, higher]],
            textposition="outside",
        ))
        fig.update_layout(
            title=f"SP vs PV – {label}",
            yaxis_title="Records",
            **_CHART_LAYOUT,
        )
        return fig
...
# User Plot Builder (for custom charts)
def _build_user_plot(
    df: pd.DataFrame,
    cfg: dict,
    dt_series: pd.Series | None,
) -> go.Figure | None:
    """Build a Plotly figure from a single plot_config dict."""
    ...
    if ct in ("line",):
        return _build_trend(df, x_col, y_cols, title, dt_series)
    if ct == "area":
        return _build_area(df, x_col, y_cols, title, dt_series)
    if ct == "bar":
        return _build_bar(df, x_col, y_cols, title)
    if ct == "scatter":
        return _build_scatter(df, x_col, y_cols, title)
    return None
...
def _build_cover(
    ...
    # ── Meta info: left-aligned, same indent as title ─────────────────────────
...
def _build_counting_section(counting: dict | None) -> list:
    ...
    donut    = _build_donut(["Auto", "Manual"], [auto_s, manual_s],
                            ["#1A56DB", "#DC2626"], "Auto / Manual")
    if donut:
        png = _fig_to_png(donut, w=460, h=260)
...
def _build_spv_section(
    ...
    for pair in spv_pairs:
        ...
        # Two-column: KV stats + bar chart
        donut_fig = _build_donut(
        ...
        donut_img = None
        if donut_fig:
            png = _fig_to_png(donut_fig,w=500,h=300)
            ...
            if img:
                ...
                story.append(t)
                story.append(Paragraph(
                    f"Distribusi Nilai Range {label}",
                    S["caption"],
                ))
        story.append(_sp(10))
    return story
...
def _build_plots_section(
    ...
    col_w   = (UW - 0.6 * cm) / 2   # width per chart cell
    col_h   = 5.2 * cm               # height per chart image
    png_w   = int(col_w / (72 / 96) * 1.6)   # approx pixel width
    png_h   = int(col_h / (72 / 96) * 1.6)

    items: list[tuple[Image | None, str]] = []
    for cfg in plot_configs:
        fig = _build_user_plot(df, cfg, dt_series)
        if fig is None:
            continue
        png = _fig_to_png(fig, w=max(png_w, 560), h=max(png_h, 240))
        img = _png_to_rl_image(png, col_w, col_h)
        cap = cfg.get("title") or f"{cfg.get('chart_type','').title()} – {cfg.get('x','')} × {', '.join(cfg.get('y') or [])}"
...
def build_pdf_reportlab(
    ...
    *,
    ...
    if duration_result:
        recorded_range = (
            f"{duration_result.get('start_time','')} – "
    ...
    # Header data to inject into canvas for page numbering and consistent header
    header_data = {
        "logo_path":      logo_path,
    ...
    # Build Story flowables for each section of the report
    story: list = []

    # 1. Cover block + dataset info
    story += _build_cover(
        display_name  = display_name,
        dataset_info  = di,
        total_rows    = len(df),
    ...
    # 5. SP vs PV — multi-pair
    story += _build_spv_section(df, spv_pairs or [], fn_compute_spv)

    # 6. User-selected charts — 2-column grid
    if plot_configs:
    ...
    # canvasmaker factory — injects header_data into each page canvas
